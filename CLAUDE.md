...
- **Version propagation bắt buộc** (RCA finding 2026-06-13, score 5.0/5) — Khi framework version thay đổi, CÁC NƠI SAU phải được cập nhật trong cùng một commit: (1) `axiom_spec.md` header; (2) hero badge `index.html` (`.sub` element, ví dụ `· v3.2`); (3) timeline strip `index.html` (`.tl-label` năm hiện tại); (4) mọi nav card description có version tag. **Không sync = propagation gap** — class lỗi đã quan sát: D1/D2/D4 (2026-06-13), hero badge "Phiên bản 2.0" tồn tại song song với v3.2 trên cùng trang. Checklist trước khi đóng PR có version bump: `grep -n "v[0-9]\|phiên bản" index.html` → tất cả entries phải thống nhất.
...
This project is indexed by GitNexus as **Mach-Re** (4937 symbols, 4725 relationships, 0 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
