...
- **BỎ:** biểu thức **giả-định-lượng** ngụ ý tính toán không tồn tại — `Resilience(S) = f(depth(V))` (f không định nghĩa), `Direction(agentᵢ) = ∇F(positionᵢ)` (gradient của F vô định). Diễn đạt lại bằng câu: *"Sức bền tăng theo chiều sâu trục mạch cội dọc."*
